...
test.describe("Shift Lifecycle", () => {
  ...
  test("complete shift lifecycle flow", async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("Start Shift")'),
    ).not.toBeVisible();
    ...
    await expect(
      page.locator(`[data-shift-id="${shiftId}"] >> text=Completed`),
    ).toBeVisible();
  });
  ...
  test("should prevent starting non-scheduled shift", async ({ page }) => {
    ...
    await expect(page.locator("text=Cannot start shift")).toBeVisible({
      timeout: 3000,
    });
  });
  ...
  test("should track break time correctly", async ({ page }) => {
    ...
    const _breakDuration = Math.floor((breakEndTime - breakStartTime) / 1000);

    // Verify break duration is tracked (should be ~3 seconds)
    const breakDurationElement = page.locator(
      "text=/Break Duration:.*[0-9]+ (seconds|minutes)/",
    );
  ...
  test("should show real-time updates via WebSocket", async ({
    page,
    context,
  }) => {
    const employeeId = await createEmployee(page);
    ...
    await expect(dashboardPage.locator("text=/Active Shifts:.*1/")).toBeVisible(
      { timeout: 5000 },
    );
    ...
    await expect(dashboardPage.locator("text=/Active Shifts:.*0/")).toBeVisible(
      { timeout: 5000 },
    );
