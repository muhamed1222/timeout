...
test.describe("Rating System", () => {
  ...
  test("should update rating after violation", async ({ page }) => {
    ...
    const initialRating = await page
      .locator('[data-testid="employee-rating"]')
      .textContent();
    ...
    await page.fill(
      ...
      "Employee started 20 minutes late without notice",
    );
  ...
  test("should create exception and restore rating", async ({ page }) => {
    ...
    const ratingAfterViolation = await page
      .locator('[data-testid="employee-rating"]')
      .textContent();
    const ratingValue = parseInt(
      ratingAfterViolation?.match(/\d+/)?.[0] || "100",
    );
    ...
    await page.fill(
      'textarea[name="description"]',
      "Traffic jam due to accident on highway",
    );
    ...
    await page.goto(`/employees/${employeeId}`);
    const restoredRating = await page
      .locator('[data-testid="employee-rating"]')
      .textContent();
  ...
  test("automatic violation detection for late start", async ({ page }) => {
    ...
    const endAt = new Date(now.getTime() + 7.5 * 60 * 60 * 1000)
      .toISOString()
      .slice(0, 16);
  ...
  test("should show company-wide rating leaderboard", async ({ page }) => {
    ...
    const _employee1Id = await createEmployee(page, {
      full_name: "John Doe",
      position: "Engineer",
    });
    const employee2Id = await createEmployee(page, {
      full_name: "Jane Smith",
      position: "Designer",
    });
    const _employee3Id = await createEmployee(page, {
    ...
    const firstEmployee = page
      .locator('[data-testid="leaderboard-entry"]')
      .first();
    ...
    const rating = await employee2Entry
