...
test.describe("Violation Management", () => {
  ...
  test("should create violation rule and add violation", async ({ page }) => {
    ...
    const createButton = page.locator(
      'button:has-text("Create Rule"), button:has-text("Создать правило")',
    );
    ...
    const addViolationButton = page.locator(
      'button:has-text("Add Violation"), button:has-text("Добавить нарушение")',
    );
  ...
  test("should verify rating updates after violation", async ({ page }) => {
    ...
    const ratingDisplay = page.locator(
      '[data-testid="rating"], .rating, text=/rating|рейтинг/i',
    );
