...
test.describe('Company Management', () => {
  ...
  test('should update company settings', async ({ page }) => {
    ...
    const settingsVisible = await page.locator('text=/settings|настройки/i').isVisible().catch(() => false);
    
    if (settingsVisible) {
      // Update timezone
      const timezoneSelect = page.locator('select[name="timezone"], input[name="timezone"]');
      if (await timezoneSelect.isVisible()) {
        await timezoneSelect.selectOption('America/New_York');
        await page.click('button[type="submit"]:has-text("Save"), button:has-text("Сохранить")');
  ...
  test('should generate shifts for company', async ({ page }) => {
    ...
    const createScheduleButton = page.locator('button:has-text("Create Schedule"), button:has-text("Создать график")');
    ...
    const generateButton = page.locator('button:has-text("Generate Shifts"), button:has-text("Сгенерировать смены")');
