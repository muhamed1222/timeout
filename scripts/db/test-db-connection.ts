...
if (!urlMatch) {
  throw new Error(
    `Failed to parse DATABASE_URL. Expected format: postgres.{projectRef}:{password}@...\n` +
    `Received URL: ${currentUrl.replace(/:[^:@]*@/, ':***@')}\n` +
    `Please ensure your DATABASE_URL follows the correct Supabase connection string format.`
  );
}
...
if (!projectRef || !password) {
  throw new Error(
    `Failed to extract required connection parameters from DATABASE_URL.\n` +
    `ProjectRef: ${projectRef ? 'present' : 'missing'}, Password: ${password ? 'present' : 'missing'}\n` +
    `URL: ${currentUrl.replace(/:[^:@]*@/, ':***@')}\n` +
    `Expected format: postgres.{projectRef}:{password}@...`
  );
}
